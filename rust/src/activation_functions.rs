...
use crate::traits::FloatLikePrimitive;
...
fn gdn_base<F: FloatLikePrimitive>(
    x: &Array3<F>,
    beta: &Array1<F>,
    gamma: &Array2<F>,
    ...
    if !inverse {
        norm = norm.mapv(|a| F::from_f64(1.0 / a.to_f64().unwrap()).unwrap());
    }
...
pub fn gdn<F: FloatLikePrimitive>(
    x: &Array3<F>,
    beta: &Array1<F>,
    gamma: &Array2<F>,
    params: GdnParameters,
...
/// We expect the data x to be passed in Pytorch layout (channels, height, width).
/// Gamma shall have the form (input channels, output channels)
pub fn igdn<F: FloatLikePrimitive>(
    x: &Array3<F>,
    beta: &Array1<F>,
    gamma: &Array2<F>,
    params: GdnParameters,
...
#[allow(dead_code)]
pub fn leaky_relu<D: Dimension, F: FloatLikePrimitive>(data: &Array<F, D>) -> Array<F, D> {
    data.mapv(|x| {
        if x.to_f64().unwrap() > 0. {
            x
        } else {
            F::from(0.01 * x.to_f64().unwrap()).unwrap()
        }
    })
}
...
/// Relu implementation
#[allow(dead_code)]
pub fn relu<D: Dimension, F: FloatLikePrimitive>(data: &Array<F, D>) -> Array<F, D> {
    data.mapv(|x| {
        if x.to_f64().unwrap() > 0. {
            x
        } else {
...
/// for more info.
pub struct GdnLayer<F: FloatLikePrimitive> {
    beta: Array1<F>,
    gamma: Array2<F>,
    parameters: GdnParameters,
}

impl<F: FloatLikePrimitive> GdnLayer<F> {
    pub fn new(beta: Array1<F>, gamma: Array2<F>, parameters: GdnParameters) -> Self {
        Self {
            beta,
            gamma,
            parameters,
    ...
    pub fn activate(&self, x: &Array3<F>) -> Array3<F> {
...
/// for more info.
pub struct IgdnLayer<F: FloatLikePrimitive> {
    beta: Array1<F>,
    gamma: Array2<F>,
    parameters: GdnParameters,
}

impl<F: FloatLikePrimitive> IgdnLayer<F> {
    pub fn new(beta: Array1<F>, gamma: Array2<F>, parameters: GdnParameters) -> Self {
        Self {
            beta,
            gamma,
...
impl ReluLayer {
    ...
    pub fn activate<F: FloatLikePrimitive>(&self, x: &Array3<F>) -> Array3<F> {
        x.map(|a| a.max(F::from(0.0).unwrap()))
    }
}
